...
Flow baru:
  1. Cari sheet utama (default: "Infra Main")
  2. Duplicate sheet utama → rename dengan timestamp backup
  3. Reset sheet utama (clear values + formatting + merge)
  4. Tulis data terbaru ke sheet utama
  5. Apply formatting ulang

Backup sheet contoh:
  Infra Backup 2026-05-11 14:30:22
...
def get_sheet_by_name(svc, spreadsheet_id: str, sheet_name: str):
    sheets = list_existing_sheets(svc, spreadsheet_id)
...
def main():
    ...
    print(f"  Nodes     : {len(nodes_data['data'])}")
    print(f"  Instances : {len(inst_data['data'])}")
